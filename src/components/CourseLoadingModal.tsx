  ...
  return (
    ...
    >
      <div
        ...
        class="bg-white border border-[#E5E7EB] w-full max-w-md rounded-2xl shadow-2xl p-6 text-center space-y-4 relative overflow-hidden"
      >
        ...
        <div class="relative w-16 h-16 mx-auto mt-2 flex items-center justify-center">
          <div class="absolute inset-0 rounded-full border-4 border-slate-100 border-t-[#1B4CA1] animate-spin"></div>
          <div class="w-10 h-10 rounded-full bg-[#1B4CA1] text-white flex items-center justify-center shadow-md">
            <BookOpen class="w-5 h-5 text-amber-300" />
          </div>
        </div>

        {/* Text Details */}
        <div>
          <span class="inline-flex items-center gap-1 text-[11px] font-bold text-[#1B4CA1] bg-[#EDF1F7] px-2.5 py-0.5 rounded-full border border-[#C7D9FB] uppercase tracking-wider mb-2">
            <ShieldCheck class="w-3.5 h-3.5 text-[#1B4CA1]" />
            <span>DoPT Verified Course</span>
          </span>
          <h3 class="text-base font-extrabold text-[#1B2133] line-clamp-1">
            {module.title}
          </h3>
        ...
        <div class="space-y-1.5 pt-1">
          <div class="flex justify-between text-[10px] font-bold text-slate-500">
            ...
            <span class="font-mono text-[#1B4CA1]">{progress}%</span>
          </div>
          <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
            <div
              class="h-full bg-[#EF951E] rounded-full transition-all duration-300 ease-out"
