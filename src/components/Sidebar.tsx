...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex lg:hidden bg-black/50 backdrop-blur-xs animate-in fade-in duration-150">
      <aside
        id="app-sidebar-mobile"
        className="w-72 bg-white border-r border-slate-200 flex flex-col h-full shadow-2xl animate-in slide-in-from-left duration-200"
      >
        {/* Mobile Header with Karmayogi Brand & Close */}
        <div className="p-4 bg-[#0F294A] text-white flex items-center justify-between border-b border-blue-900">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-md bg-[#FF7722] flex items-center justify-center font-extrabold text-white text-lg">
              K
            </div>
            <div>
              <div className="flex items-center gap-1 font-bold text-sm">
                <span>iGOT</span>
                <span className="text-orange-400">KARMAYOGI</span>
              </div>
              <p className="text-[10px] text-blue-200">Ira AI Capacity Engine</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className={`text-[9px] px-2 py-0.5 rounded-full font-mono font-bold ${item.badgeColor}`}>
                    {item.badge}
                  </span>
                )}
              </button>
        ...
        <div className="p-3.5 m-3 rounded-xl bg-orange-50/70 border border-orange-200 text-xs text-orange-950">
          ...
          <div className="mt-2 pt-2 border-t border-orange-200/80 flex items-center justify-between text-[10px] text-orange-700">
            <span>DoPT Verified</span>
            <span className="font-mono">v2.4 Live</span>
          </div>
        </div>
      </aside>
    </div>
