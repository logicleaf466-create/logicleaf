...
export const LibraryScreen: React.FC<LibraryScreenProps> = ({
  ...
  return (
    <section id="library-screen" className="space-y-8 animate-in fade-in duration-200 pb-16">
      ...
      <div className="bg-[#EDF1F7] p-6 rounded-2xl border border-[#C7D9FB] shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#1B4CA1] font-bold flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-[#EF951E]" /> DoPT Accredited National Catalog
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="px-4 py-2 bg-[#EF951E] hover:bg-[#F08811] text-white rounded-xl text-xs font-bold shadow-xs transition-colors cursor-pointer"
          >
      ...
      <div className="bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-2xs space-y-4">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full text-xs pl-10 pr-4 py-2.5 rounded-xl border border-[#C7D9FB] bg-[#FEFAF4] text-[#1B2133] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#1B4CA1]"
            />
          </div>
          ...
          {searchQuery && (
            <button
              ...
              className="text-xs text-red-600 hover:underline font-bold px-2 py-1 cursor-pointer shrink-0"
            >
              Reset Search
            </button>
          )}
        ...
        <div className="flex items-center gap-1.5 flex-wrap text-[11px] pt-2 border-t border-slate-100">
          <span className="text-slate-400 font-semibold mr-1">Official Topics:</span>
          {[
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-[#EDF1F7] hover:bg-[#FFE9CD] text-[#1B4CA1] hover:text-[#C37024] border border-[#C7D9FB] hover:border-[#FFD2A1] transition-colors cursor-pointer text-[10px] font-semibold"
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
          <span className="text-[11px] font-bold text-[#4B5563] uppercase tracking-wider shrink-0 mr-1">
            Domains:
          </span>
          {domains.map((dom) => (
            <button
      ...
      <div id="showcased-courses-section" className="bg-white rounded-2xl border border-[#E5E7EB] p-5 sm:p-7 shadow-xs">
        {/* Section Header */}
        <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <h3 className="text-xl sm:text-2xl font-black text-[#1B2133] tracking-tight">
              Showcased Courses
            </h3>
            <span className="hidden sm:inline-block text-[11px] font-bold bg-[#FFE9CD] text-[#C37024] border border-[#FFD2A1] px-2.5 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-xs sm:text-sm font-bold text-[#1B4CA1] hover:text-[#EF951E] flex items-center gap-1 transition-colors group cursor-pointer"
            >
              <span>Show all</span>
            ...
            <div className="flex items-center gap-1.5 ml-2">
              <button
                ...
                className="w-8 h-8 rounded-full bg-slate-900 hover:bg-slate-700 text-white flex items-center justify-center transition-all shadow-xs cursor-pointer active:scale-95"
              >
        ...
        <div className="p-3.5 bg-gradient-to-r from-[#EDF1F7] to-amber-50 rounded-xl border border-[#C7D9FB] flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 text-[#1B4CA1]">
            <Database className="w-4 h-4 text-[#EF951E] shrink-0" />
            <span>
          ...
          <span className="text-[10px] font-bold font-mono bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded border border-emerald-300 flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Backend Synchronized
          </span>
        </div>

        {/* Course Cards Carousel / Row */}
        {filteredModules.length === 0 ? (
          <div className="text-center py-12 px-4 bg-[#FEFAF4] rounded-xl border border-dashed border-[#FFD2A1]">
            <p className="text-sm font-bold text-[#1B2133]">No showcased courses found</p>
            ...
            <button
              ...
              className="mt-3 px-3.5 py-1.5 bg-[#1B4CA1] text-white text-xs font-bold rounded-lg cursor-pointer hover:bg-[#1146A2] transition-colors"
            >
              Reset Filters
            </button>
          </div>
          ...
          >
            {filteredModules.map((mod) => {
              ...
              return (
                <div
                  key={mod.id}
                  className="w-[285px] sm:w-[310px] shrink-0 snap-start bg-white rounded-xl border border-[#E2E8F0] hover:border-[#1B4CA1] shadow-2xs hover:shadow-md transition-all flex flex-col justify-between overflow-hidden group"
                >
                  <div>
                    {/* Visual Course Thumbnail */}
                    <div className="relative cursor-pointer" onClick={() => onOpenModule(mod)}>
                      <CourseCardThumbnail
                      ...
                      <button
                        ...
                        className="absolute top-2.5 right-2.5 p-1.5 rounded-full bg-black/40 hover:bg-black/70 text-white transition-colors cursor-pointer z-10"
                        aria-label="Bookmark course"
                      >
                        <Bookmark className={`w-3.5 h-3.5 ${isSaved ? 'fill-[#EF951E] text-[#EF951E]' : ''}`} />
                      </button>
                    </div>
                    ...
                    <div className="p-4 space-y-2.5">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="inline-flex items-center gap-1 bg-[#FFF4E5] border border-[#FFD2A1] text-[#E07D10] text-[10px] font-extrabold px-2.5 py-0.5 rounded-full">
                          <Video className="w-3 h-3 text-[#E07D10] fill-current" />
                          <span>Course</span>
                        </span>
                      ...
                      <h4
                        ...
                        className="text-sm font-extrabold text-[#1B2133] group-hover:text-[#1B4CA1] transition-colors line-clamp-2 min-h-[40px] leading-snug cursor-pointer"
                      >
                        {mod.title}
                      </h4>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {mod.extractedVideos && mod.extractedVideos.length > 0 ? (
                          ...
                          <span className="text-[10px] text-blue-800 font-bold bg-blue-50 px-2 py-0.5 rounded border border-blue-200 inline-flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
                        ...
                        <p className="text-[11px] text-[#4B5563] font-medium line-clamp-1">
                          By {mod.provider}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="p-4 pt-0">
                    <button
                      ...
                      onClick={() => onOpenModule(mod)}
                      className="w-full py-2.5 px-3 rounded-lg text-xs font-bold bg-[#EF951E] hover:bg-[#F08811] text-white flex items-center justify-center gap-1.5 shadow-2xs hover:shadow-xs transition-all cursor-pointer"
                    >
